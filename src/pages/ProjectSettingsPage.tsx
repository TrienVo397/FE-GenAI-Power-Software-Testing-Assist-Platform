...
const ProjectSettingsPage = () => {
  ...
  const [isUpdatingCurrent, setIsUpdatingCurrent] = useState<boolean>(false);
  ...
  const fetchVersions = async (projectId: string) => {
    setIsLoading(true);
    try {
      const versionsList: DocumentVersion[] = await getDocumentVersionsByProject(projectId);
  ...
  const handleSetCurrentVersion = async (versionId: string, versionLabel: string) => {
    if (!currentProject?.id || isUpdatingCurrent) return;
          ...
          {isLoading ? (
            ...
            <div className="bg-gray-50 p-8 text-center rounded-md">
              ...
              <p className="mt-2 text-sm text-gray-400">
                Click "Create Version" to add a new version.
              </p>
            </div>
          )}
        </div>
