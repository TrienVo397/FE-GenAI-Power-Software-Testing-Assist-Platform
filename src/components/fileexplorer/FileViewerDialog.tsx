...
const FileViewerDialog: React.FC<FileViewerDialogProps> = ({
  ...
  const isEditable =
    ...
    ['md', 'yml', 'yaml', 'txt', 'json', 'csv', 'html', 'js', 'py', 'xml'].includes(
      _.toLower(_.last(file.name.split('.')) || '')
    );

  if (!file) return null;

  const fileSize = file.size ? parseInt(file.size.toString()).toLocaleString() : 'Unknown';
      ...
      <div className="w-full">
        ...
        {viewMode === 'binary' && (
          <div className="text-center p-4">
            ...
            <div className="text-sm text-gray-500">File size: {fileSize} bytes</div>
          </div>
        )}
        ...
        <div className="mt-4 text-sm">
          ...
          <div><strong>Last modified:</strong> {new Date(file.last_modified).toLocaleString()}</div>
        </div>
      </div>
    </SimpleDialog>
  );
